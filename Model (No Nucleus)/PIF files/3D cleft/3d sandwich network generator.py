...
output_file = "./3d collagen60000_threeparts_sandwhich_network_nooverlap_nocluster_seperated cleft_constantratio_13.piff"
...
n_f_pd_op = 400000	#number of fibres per direction for the sandwhich, outer parts and cleft
n_f_c = 100000
...
with open(output_file, 'w') as f:
    ...
    for cell_id in range(3 + n_f_pd_sw + n_f_c + n_f_pd_op , 3 + 2* n_f_pd_sw + n_f_c + n_f_pd_op):
        ...
        if (p==0):
            ...
            y = random.randint((lattice_dim/2)+cleft_hight, (lattice_dim/2)+sw_hight+cleft_hight -fibre_length_sw_y- 1)
            z = random.randint(0, lattice_dim - 1)
        if (p==1):
            x = random.randint(0, lattice_dim - 1)
            y = random.randint((lattice_dim/2)-cleft_hight-sw_hight, (lattice_dim/2)- cleft_hight - fibre_length_sw_y - 1)
            z = random.randint(0, lattice_dim - 1)
