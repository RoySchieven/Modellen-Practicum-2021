...
output_file = "C:\\users\\roysc\\downloads\\ru-modellenpracticum-master\\ru-modellenpracticum-master\\Model\\PIF files/3d cleft40000 ratio3 nucl.piff" # make sure to use the right path
...
with open(output_file, 'w') as f:
    ...
    f.write("0 Medium 0 0 " + str(lattice_dim - 1) + " 0 " + str(lattice_dim - 1) + " 0 " + str(lattice_dim-1) + '\n')
    ...
    f.write(" ".join([str(0), str(0), "Medium", str(lattice_dim//2-8), str(lattice_dim//2+8), str(0), str(lattice_dim - 1), str(0), str(lattice_dim-1), '\n']))
